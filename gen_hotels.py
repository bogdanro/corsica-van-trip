...
def _eat_days(EAT_MAP):
    ...
    return day_eat, pins
from eats import HOTEL_EAT
DAY_EAT, EAT_PINS = _eat_days(HOTEL_EAT)
...
for r in routes:
    ...
    days.append(dict(day=r["day"], title=r["title"], km=r["km"], min=r["min"],
                     ...
                     flow=FLOW.get(r["day"], []), eat=DAY_EAT.get(r["day"]), photos=ph))
...
for _p in pois_out:
    if _p.get("c") != "food" and _p["id"] in CORROB:
        _p["src"] = CORROB[_p["id"]]
out = dict(pois=pois_out + EAT_PINS, camps=[], stays=STAYS, days=days)
